...
#### What you need
Create a **new GitHub repository** and add **two files**:

| File | Path |
|------|------|
| 📄 `renew.py` | root of the repo |
| ⚙️ `renew.yml` | `.github/workflows/renew.yml` |

> Full content of both files is inside `Time Ultimited.txt` in this project.

#### Setup steps

**1. Create the two files**
- Put `renew.py` in the repo root  
- Put `renew.yml` in `.github/workflows/`
...
| `SERVER_ID` | Your server ID (from the server URL) |

**3. Workflow runs automatically**
- Every **3 days** (schedule in `renew.yml`)
- Or run manually: **Actions** → **Parham Confing Auto Renew** → **Run workflow**

The script logs into Katabump, opens your server, and clicks **Renew** when the button is available.
...
📦 Panel - VPS
 ┣ 📜 index.js
 ┣ 📜 package.json
 ┣ 📜 Time Ultimited.txt    ← renew.py + workflow guide
 ┗ 📁 banner
    ┗ 🖼️ banner.png
```
...
می‌توانید با **GitHub Actions** این کار را خودکار کنید.

#### چی لازم دارید؟
یک **ریپازیتوری جدید** در گیت‌هاب بسازید و **دو فایل** داخلش قرار دهید:
...
> متن کامل هر دو فایل داخل `Time Ultimited.txt` همین پروژه است.

#### مراحل راه‌اندازی

**۱. ساخت دو فایل**
- فایل `renew.py` را در ریشه ریپو بگذارید  
- فایل `renew.yml` را در مسیر `.github/workflows/` بگذارید
...
**۳. اجرای خودکار Workflow**
- هر **۳ روز** یک‌بار طبق زمان‌بندی `renew.yml`
- یا دستی: **Actions** → **Parham Confing Auto Renew** → **Run workflow**

اسکریپت وارد کاتابامپ می‌شود، سرور را باز می‌کند و اگر دکمه **Renew** فعال باشد روی آن کلیک می‌کند.
...
📦 Panel - VPS
 ┣ 📜 index.js
 ┣ 📜 package.json
 ┣ 📜 Time Ultimited.txt    ← راهنمای renew.py و workflow
 ┗ 📁 banner
    ┗ 🖼️ banner.png
```
